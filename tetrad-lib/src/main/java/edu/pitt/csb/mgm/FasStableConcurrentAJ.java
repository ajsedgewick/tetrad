 ...
 */
public class FasStableConcurrentAJ implements IFas {

    /**
     * The independence test. This should be appropriate to the types
     */
    private IndTestMixedMultipleTTest test;
     ...
     */
    public FasStableConcurrentAJ(IndTestMixedMultipleTTest test) {
        this.test = test;
    }
...
//    }
    ...
    private boolean searchAtDepth(final List<Node> nodes, final IndTestMixedMultipleTTest test, final Map<Node, Set<Node>> adjacencies,
                                  final int depth) {
        ...
        class DepthTask extends RecursiveTask<Boolean> {
            ...
            protected Boolean compute() {
                if (to - from <= chunk) {
                    Map<Set<Node>, Set<Edge>> testTracker = new HashMap<>();
                    Set<Edge> remEdges = new HashSet<>();
                    ...
                    for (int i = from; i < to; i++) {
                        ...
                        //keep track if we've already done a test... this may end up taking a lot of memory
                        //index is on conditioning set

                        EDGE:
                        for (Node y : adjx) {
                            Edge xy = new Edge(x,y,Endpoint.NULL,Endpoint.NULL);
                            if(remEdges.contains(xy)){
                                continue EDGE;
                            }
                            ...
                            if (ppx.size() >= depth) {
                                ...
                                while ((choice = cg.next()) != null) {
                                    ...
                                    List<Node> condList = GraphUtils.asList(choice, ppx);
                                    HashSet<Node> condSet = new HashSet<>(condList);
                                    //for(int ci = 0; ci < choice.length; ci++){ conds.add(ci);}
                                    Set<Edge> edgeSet = testTracker.get(condSet);
                                    if(edgeSet != null && edgeSet.contains(xy)) {
                                        //System.out.println("Skipped a Redundant test!");
                                        continue EDGE;
                                    }
                                    ...
                                    try {
                                        ...
                                        //dependantPvals correspond to [y, condSet]
                                        pvec = test.dependencePvals(x,y,condList);
                                    ...
                                    if (pvec != null){
                                        //&&} noEdgeRequired) {
                                        List<Node> regressors = new ArrayList<Node>(condList.size()+1);
                                        Node dep;
                                        if(test.getFlipLast() == true){
                                            dep = y;
                                            regressors.add(x);
                                        } else {
                                            dep = x;
                                        ...
                                        for(int pi = 0; pi < pvec.length; pi++) {
                                            ...
                                            Set<Node> curCondSet = new HashSet<Node>(curCond);
                                            Edge curEdge = new Edge(dep,curNode,Endpoint.NULL,Endpoint.NULL);

                                            if(pvec[pi] > test.getAlpha()) {
                                                remEdges.add(curEdge);

                                                boolean noEdgeRequired =
                                                        knowledge.noEdgeRequired(dep.getName(), curNode.getName());

                                                adjacencies.get(dep).remove(curNode);
                                                adjacencies.get(curNode).remove(dep);

                                                getSepsets().set(dep, curNode, curCond);

                                                if (verbose) {
                                                    TetradLogger.getInstance().log("independencies", SearchLogUtils.independenceFact(dep, curNode, condList) + " p = " +
                                                            nf.format(test.getPValue()));
                                                    out.println(SearchLogUtils.independenceFactMsg(dep, curNode, curCond, pvec[pi]));
                                                }
                                            }

                                            if(testTracker.containsKey(curCondSet))
                                                ((HashSet) testTracker.get(curCondSet)).add(curEdge);
                                            else {
                                                HashSet<Edge> curSet = new HashSet<>();
                                                curSet.add(curEdge);
                                                testTracker.put(curCondSet, curSet);
                                            }

                                        }

                                        if(pvec[0] < test.getAlpha())
                                            continue EDGE;
                                    }
                                }
                            }
